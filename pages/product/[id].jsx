        ...
        <div className={styles.right}>
            ...
            <div className={styles.sizes}>
                <div className={styles.size} onClick={()=>handleSize(0)}>
                    ...
                    <span className={size===0? styles.active : styles.pizzaSize}>Small</span>
                </div>
                <div className={styles.size} onClick={()=>handleSize(1)}>
                    <Image src="/img/size.png" fill alt=""/>
                    <span className={size===1? styles.active : styles.pizzaSize}>Medium</span>
                </div>
                <div className={styles.size} onClick={()=>handleSize(2)}>
                    <Image src="/img/size.png" fill alt=""/>
                    <span className={size===2? styles.active : styles.pizzaSize}>Large</span>
                </div>
            </div>
...
export const getServerSideProps = async ({params}) => {
    const res = await axios.get(process.env.BASE_API + "products/" + params.id)
    return{
        props:{
        pizza: res.data
        }
